...
describe('Social Controller', () => {
  ...
  test('GET /api/socials/active should throw exception', async () => {
    jest.spyOn(Social, 'find').mockImplementation(() => {
      throw new Error('Error');
    });
    await supertest(app)
  ...
  test('GET /api/socials/active', async (done) => {
    ...
    await supertest(app)
      ...
      .then(({ body }) => {
        ...
        expect(body[0]._id).toBe(social1.id);
        expect(body[0].name).toBe(social1.name);
        expect(body[0].active).toBeTruthy();
        expect(body[1]._id).toBe(social2.id);
        expect(body[1].name).toBe(social2.name);
        expect(body[1].active).toBeTruthy();
      })
      .finally(() => done());
  });
